...
class CreateSitemapController extends BaseAdmin
{
    ...
    public function inputData($links_counter = 1, $redirect = true)
    {
        ...
        if (!function_exists('curl_init')) {
            $this->cancel(0, 'Library CURL is absent. Create sitemap impossible.', '', true);
        }
        ...
        do {
            ...
            if (false !== $info) {
                ...
                if ($info['result'] !== 0) {
                    ...
                    if ($error != 0) {
                        $this->cancel(0,
                            'Error loading ' . $header['url'] . ' http code: ' . $header['http_code'] .
        ...
        if ($content) {
            ...
            if ($links[2]) {
                ...
                foreach ($links[2] as $link) {
                    ...
                    if (!in_array($link, $this->bad_links) && !preg_match('/^(' . $site_url . ')?\/?#[^\/]*?$/ui',
                            $link) &&
