...
abstract class BaseModel extends BaseModelMethods
{
     ...
     * fields => [поле => занчение]; если не указан, то обрабатывается $_POST[поле => значение]
     * разрешена передача например NOW() в качестве MySQL функции обычно строкой
     * files => [поле => занчение]; помжно подать массив вида [поле => [массив значений]]
     * except => [исключение 1, исключение 2] - исключает данные эл-ты массива из добавления в запрос
    ...
    final public function add($table, $set = [])
    {
        ...
        //TODO $set['except'] = (is_array($set['except']) && !empty($set['except'])) ? $set['except'] : false;
